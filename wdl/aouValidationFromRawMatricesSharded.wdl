...
workflow aouArrayValidation {
    ...
    scatter (i in range(length(contigs))) {
        ...
        call FillMissingValues {
            input:
                raw_matrix = raw_lrr_by_contig[i],
        ...
        call gsirs_sharded.GenomeStripIRSSharded as GenomeStripIRSSharded {
            input:
                ...
                filled_lrr=FillMissingValues.filled_lrr,
...
task subsetGATKSV {
	...
	command <<<
        ...
        bcftools reheader --samples ~{sample_map} ~{gatk_sv_vcf} -o ~{prefix}.~{chromosome}.reheader.vcf.gz
        tabix ~{prefix}.~{chromosome}.reheader.vcf.gz
        bcftools view ~{prefix}.~{chromosome}.reheader.vcf.gz \
            -r ~{chromosome} \
            -S ~{sample_list} \
